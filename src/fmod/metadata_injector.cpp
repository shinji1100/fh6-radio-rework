...
bool write_string_slot(std::byte* target, std::string_view src) noexcept {
    ...
    // Snapshot the existing header. SEH-wrapped: a stale page would otherwise
    // take the game's audio thread down with us.
    StringHeader hdr{};
    ...
    auto allocate_and_swap = [&]() -> bool {
        // Round up: (size + 32) & ~15. Gives the game a little headroom for
        // in-place follow-up writes.
        ...
        // Switch the slot: pointer first, then size, then cap. A racing reader
        // picks up either the old (still-valid) buffer or the new one; never a
        // mismatched (ptr, size, cap) trio. The old buffer leaks deliberately.
        if (!seh_call([&] {
