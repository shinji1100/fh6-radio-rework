...
3. 六视角由 CameraTracker 闭环几何跟踪驱动（见第 5 节），不再是开环计数。
4. 再做实时 A/B、响度校准、实测 HRTF、车辆类别拟合和敞篷状态。

本地 HEAD 是 `4634e58`（闭环跟踪器根因修复，**尚未推送**：本机无 GitHub 凭据，MCP 写 403）。其下依次是 `3aa44e3`（循环顺序修正 + sync API）和远端最新的 `5622841`。游戏根目录当前安装的是 `5622841`（旧开环方案），新构建需先推送触发 CI。
